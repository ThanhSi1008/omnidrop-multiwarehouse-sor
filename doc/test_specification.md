...
- **Phiên bản:** v1.1 (Full Coverage Specification)
- **Tác giả:** Kỹ sư Trưởng / QA Lead
- **Trạng thái:** Hoàn tất Ma trận Test Cases cho 100% Microservices
...
- **`api-gateway` (Port 3000):** Tested in `TC-GW-01`, `TC-GW-02`, `TC-BOUND-01`, `TC-CONC-01`.
- **`core-service` (Port 3001 & gRPC 50052):** Tested in `TC-CORE-01`, `TC-CORE-02`, `TC-INT-01`, `TC-INT-03`.
...
| `TC-UT-04` | Phân vùng Địa lý Kho Ưu tiên Miền Bắc (Geographic Mapping) | `order-routing-service` (SOR Helper) | Province = "Hanoi" / "Hai Phong" | Tra về `primary = KHO_HN`, `secondary = KHO_HCM`. |
...
### 2. Cấp độ: Integration Testing (INT)

| Test Case ID | Tên Test Case | Thành phần Target | Dữ liệu Đầu vào | Kết quả Kỳ vọng |
| :--- | :--- | :--- | :--- | :--- |
| `TC-INT-01` | Truy vấn gRPC Stock Check (`GetSkuStock`) | `order-routing-service` &rarr; `core-service` | SKU = `KINH-X-DEN-SIZE-M` | gRPC Client nhận được mảng `stocks` chứa `quantity`, `reservedQuantity` và `price` chuẩn `keepCase: true`. |
| `TC-INT-02` | Bắn và Tiêu thụ Event `order.created` qua RabbitMQ | `flash-sale-service` &rarr; RabbitMQ &rarr; `order-routing-service` | Event Payload hợp lệ | Message tới queue `order_created_queue`, `order-routing-service` tiêu thụ và tạo bản ghi Order trong DB. |
| `TC-INT-03` | Bắn và Tiêu thụ Event Saga `order.paid` | `order-routing-service` &rarr; RabbitMQ &rarr; `core-service` | Event `order.paid` với deductions | `core-service` nhận event, trừ số lượng tồn kho vật lý và giải phóng `reservedQuantity` trong PostgreSQL schema `core`. |
| `TC-INT-04` | Bắn và Tiêu thụ Event Saga `order.timeout` | `order-routing-service` &rarr; RabbitMQ &rarr; `flash-sale-service` | Event `order.timeout` | `flash-sale-service` nhận event, dùng Redis Multi `INCRBY` nhả lại kho và `DEL` xóa key `user:limit`. |
...
| `TC-CORE-01` | Lấy danh sách sản phẩm & Tồn kho đa kho | `core-service` | `GET /products` | Trả về mảng biến thể kèm chi tiết tồn kho vật lý và ATS của `KHO_HN` & `KHO_HCM`. |
| `TC-CORE-02` | Xử lý An toàn Event `order.paid` khi SKU/Warehouse không tồn tại | `core-service` | Event `order.paid` chứa SKU giả | Log error thông báo không tìm thấy SKU, không văng ngoại lệ làm crash service. |
...
| `TC-ORDER-01` | Lấy danh sách Đơn hàng & Fulfillments | `order-routing-service` | `GET /orders` | Trả về mảng danh sách đơn hàng đã xếp theo thời gian tạo mới nhất (`createdAt DESC`). |
...
| :--- | :--- | :--- | :--- | :--- |
| `TC-E2E-01` | Mua hàng & Thanh toán thành công (Happy Path) | 1. `POST /purchase` (User 1, Hanoi)<br>2. Chờ SOR routing<br>3. `POST /orders/:id/pay` | PostgreSQL `order` & `core` tables, Redis stock | - Order lưu trạng thái `PAID`<br>- Fulfillment được gán kho `KHO_HN`<br>- Physical Stock `KHO_HN` giảm 1 đơn vị trong DB. |
| `TC-E2E-02` | Mua hàng Quá hạn & Hủy đơn tự động (Timeout Rollback) | 1. `POST /purchase` (User 2, HCM)<br>2. Cập nhật created_at quá 5p<br>3. Chạy Timeout Scanner | PostgreSQL order status, Redis campaign stock | - Order lưu trạng thái `CANCELLED`<br>- Tồn kho Redis được hoàn trả +1<br>- Key giới hạn mua user 2 bị xóa. |
...
| `TC-SOR-02` | SOR Chuyển hướng Kho Phụ (Secondary Warehouse Fallback) | Đặt hàng địa chỉ Hà Nội khi Kho Hà Nội hết hàng nhưng Kho HCM còn hàng | PostgreSQL `fulfillments` table | Gán 1 Fulfillment xuất từ `KHO_HCM`. |
| `TC-SOR-03` | SOR Tách đơn Tự động (Split Order Execution) | Đặt hàng nhiều sản phẩm mà không kho nào ôm trọn giỏ hàng | PostgreSQL `fulfillments` table | Tự động tạo 2 bản ghi Fulfillment độc lập (1 từ `KHO_HN`, 1 từ `KHO_HCM`). |
...
| `TC-CONC-02` | Đua mua trùng lặp của 1 User (Single User Spurt) | 50 requests song song từ cùng 1 `userId` | Redis `user:limit` & DB orders | - Chỉ 1 request đầu tiên thành công<br>- 49 requests còn lại nhận lỗi `User purchase limit exceeded`<br>- User chỉ có duy nhất 1 đơn trong DB. |

---

### 6. Cấp độ: Boundary & Exception Testing (BOUND)

| Test Case ID | Tên Test Case | Dữ liệu Gửi lên | API Endpoint | Kết quả Kỳ vọng |
| :--- | :--- | :--- | :--- | :--- |
| `TC-BOUND-01` | Thiếu trường bắt buộc trong Request Payload | Thiếu `deliveryAddress` hoặc `paymentMethod` | `POST /purchase` | Trả về HTTP 400 Bad Request: `Missing required fields`. |
| `TC-BOUND-02` | Thanh toán Đơn hàng không tồn tại | ID order giả `invalid-uuid` | `POST /orders/invalid-uuid/pay` | Trả về HTTP 404 Not Found: `Order not found`. |
| `TC-BOUND-03` | Thanh toán Đơn hàng đã được Thanh toán trước đó | Gọi API pay 2 lần liên tiếp cho 1 Order | `POST /orders/:id/pay` | Lần 1 thành công (HTTP 200), Lần 2 báo lỗi trạng thái đơn không phải `PENDING_PAYMENT`. |
| `TC-BOUND-04` | Cấu hình Admin Stock số lượng âm | Body: `{ sku: "SKU", stock: -5 }` | `POST /purchase/admin/campaign` | Trả về HTTP 400 Bad Request: `Invalid sku or stock`. |
...
3. `tests/unit/services-endpoints.spec.js`: Test unit & integration cho các API Endpoints của 4 microservices (`TC-GW-01`, `TC-GW-02`, `TC-CORE-01`, `TC-FLASH-01`, `TC-FLASH-02`, `TC-ORDER-01`, `TC-ORDER-02`).
...
8. `tests/run_all_tests.js`: Master test runner chạy 100% bộ test và đối soát kết quả.
